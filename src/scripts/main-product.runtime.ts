...
type VariantJson = {
  ...
  price_cents: number
  price_display_pdp?: string
  compare_at_display_pdp?: string | null
...
function initPdpAccordionsGsap(
  container: HTMLElement,
  signal: AbortSignal,
  reduced: boolean
): void {
  ...
  const CloseEase = 'power3.inOut'
  const DurOpen = 0.52
  const DurClose = 0.44

  const detailsList = container.querySelectorAll<HTMLDetailsElement>('[data-pdp-accordion]')
  for (const details of detailsList) {
    const inner = details.querySelector<HTMLElement>('.main-product__accordion-panel-inner')
    if (!inner) continue

    details.classList.add('main-product__accordion--gsap')
    ...
    details.addEventListener(
      ...
      (e) => {
        ...
        if (!details.open) {
          ...
          gsap.set(inner, { height: current, overflow: 'hidden' })
          gsap.to(inner, {
            height: 0,
            duration: DurClose,
            ease: CloseEase,
...
function cleanupPdpAccordionsGsap(container: HTMLElement): void {
  container.querySelectorAll<HTMLDetailsElement>('[data-pdp-accordion]').forEach((details) => {
    const inner = details.querySelector<HTMLElement>('.main-product__accordion-panel-inner')
    if (inner) {
      gsap.killTweensOf(inner)
      gsap.set(inner, { clearProps: 'height,overflow' })
    }
    details.classList.remove('main-product__accordion--gsap')
  })
}
...
export function attachMainProduct(container: HTMLElement): void {
      ...
      initPdpAccordionsGsap(container, signal, reduced)
      ...
      function applyVariant(id: string): void {
        ...
        const updateDom = (): void => {
          if (v.price_display_pdp) {
            priceEl.innerHTML = v.price_display_pdp
          } else {
            priceEl.textContent = v.price
          }
          if (compareEl) {
            if (v.compare_at_display_pdp) {
              compareEl.innerHTML = v.compare_at_display_pdp
      ...
      extended.__mainProductTeardown = () => {
        ...
        cleanupPdpAccordionsGsap(container)
        abort.abort()
...
export function detachMainProduct(container: HTMLElement): void {
  const extended = container as HTMLElement & { __mainProductTeardown?: MainProductTeardown }
  extended.__mainProductTeardown?.()
